...
class Client(object):
    ...
    def setattr(self, appid, sid, skey, bucket):
        if len(appid) != 0:
            self._auth.setattr_appid(appid)
        if len(sid) != 0:
            self._auth.setattr_secretid(sid)
        if len(skey) != 0:
            self._auth.setattr_secretkey(skey)
        if len(bucket) != 0:
            self._bucket = bucket
    ...
    def idcard_detect(self, pictures, cardType=0):
        ...
        '''
        if not isinstance(pictures, CIUrls) and not isinstance(pictures, CIFiles) and not isinstance(pictures,
                                                                                                     CIBuffers):
    ...
    def general_detect(self, pictures):
        """ 通用印刷体识别
        Note:仅识别一张图
        :param pictures: 需要添加的图片
        :type  pictures: CIFiles or CIBuffers or CIUrls
    ...
    def handwriting_detect(self, pictures):
        """ 手写体识别
        Note:仅识别一张图
        :param pictures: 需要添加的图片
        :type  pictures: CIFiles or CIBuffers or CIUrls
        ...
        requrl = self._conf.build_url('/ocr/handwriting')
        headers = {
            'Host': self._conf.host(),
            'Authorization': self._auth.get_sign(self._bucket),
            'User-Agent': Conf.get_ua(self._auth._appid),
    ...
    def bankcard_detect(self, pictures):
        """ 银行卡识别
        Note:仅识别一张图
        :param pictures: 需要添加的图片
        :type  pictures: CIFiles or CIBuffers or CIUrls
        ...
        requrl = self._conf.build_url('/ocr/bankcard')
        headers = {
            'Host': self._conf.host(),
            'Authorization': self._auth.get_sign(self._bucket),
            'User-Agent': Conf.get_ua(self._auth._appid),
